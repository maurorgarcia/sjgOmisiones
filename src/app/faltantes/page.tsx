    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-white tracking-tight uppercase">Gestión de Faltantes</h1>
            <p className="text-slate-500 text-xs font-medium uppercase tracking-widest mt-0.5">Control independiente de personal faltante en registros.</p>
          </div>
        </div>
        <Link
          href="/faltantes/carga"
          className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 px-5 py-2.5 rounded-2xl text-[10px] font-black text-black uppercase tracking-widest shadow-lg hover:shadow-[0_0_20px_rgba(245,158,11,0.3)] hover:scale-[1.02] transition-all active:scale-95"
        >
            ...
            <thead className="text-[10px] text-slate-500 uppercase bg-black/60 border-b border-white/5 tracking-[0.2em] font-black">
              <tr>
                <th className="px-6 py-4">
                  <button onClick={() => handleSort('fecha')} className="flex items-center gap-1.5 hover:text-amber-500 transition-colors">
                    Fecha
                ...
                <th className="px-6 py-4">
                  <button onClick={() => handleSort('nombre_apellido')} className="flex items-center gap-1.5 hover:text-amber-500 transition-colors">
                    Empleado
                ...
                filtered.map((f) => (
                  <tr key={f.id} className="hover:bg-white/5 transition-all group border-b border-white/5 last:border-0">
                    ...
                    <td className="px-6 py-5 text-right">
                      ...
                      >
                        {deletingId === f.id ? <Loader2 className="w-4 h-4 animate-spin text-amber-500" /> : <Trash2 className="w-4 h-4 group-hover/btn:drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]" />}
                      </button>
                    </td>
                  </tr>
                ))
